...
Bilby 由一个人维护，组织名不代表这是一个委员会。它的形态来自一份对视频客户端应有行为
的判断，这里维护的正是这份判断，方向上的决定由维护者做出。

修复 bug、报告崩溃和小规模更正，可以直接提交 PR。

新增功能和行为变更需要先经过 RFC：开一个 issue，说明想要什么、应用目前如何处理这件事、
预期的设计是什么，达成一致后再开始写代码。本文开头的约束是最可能受到影响的部分；未经
RFC 达成一致就改动这些约束的 PR，仅凭这一点就会被关闭，与代码质量无关。

被关闭不代表这个想法没有价值，只代表它不属于这个项目。本项目是 GPL-3.0，fork 出去按
自己的判断做是完全正当的，也不需要谁批准。
