...
Bilby 是一个 bilibili Android 客户端。首页只有关注 UP 主的更新，按时间倒序，向下滑动
只会到达更早的内容。没有推荐流、相关视频栏与自动连播。
...
播放页上，官方客户端放推荐的位置改为该视频所属的合集与这位 UP 主的其他投稿：一个打开
视频时就已确定的集合，播完即停。

想看别的需要主动开口——直接搜索，或者交给助理。助理会检索、读简介与热评，给出若干
视频并说明理由；它的上下文只有本次会话，不含你的观看历史。

历史记录、投币、收藏、点赞照常提交至 bilibili，去掉的只是平台向你推送内容的部分。
