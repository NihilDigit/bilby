...
播放支持全屏、切换清晰度、倍速、长按快进、拖动进度条、锁定、双击暂停与播放、多 P
视频。SponsorBlock 片段默认跳过。

听视频与看视频共用同一个播放器，切换只换界面：声音不断、位置不变，切过去之后不再
下载画面。息屏或划走界面后继续播放，通知栏、锁屏与耳机线控均可控制，来电或拔出耳机
时自动暂停，并支持定时关闭。在 UP 主空间也可以直接开始听其投稿。

评论支持浏览、排序、展开楼中楼、发布、点赞与删除。
