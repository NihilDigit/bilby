...
欢迎贡献。修复 bug、报告崩溃、补充文档和小规模更正，直接提交 PR 即可。

新增功能和破坏性改动**建议先开一个 RFC issue**，说明想要什么、应用目前如何处理这件事、
预期的设计是什么。这不是审批环节，是为了避免方向不一致导致的返工——本文开头那几条约束
是固定的，改动它们的 PR 大概率不会被合并，而代码写完之后才发现这件事，浪费的是你的时间。

**允许使用 LLM 辅助编码**，要求只有一条：理解自己提交的代码在业务上做了什么。能说清它
为什么可行、会影响到哪些地方就够了，不要求每一行都是手写的。

许可证由上游依赖决定：接口层移植自 PiliPlus（GPL-3.0），因此 Bilby 不可能迁出 GPL。
本项目不使用 CLA，你保留自己那部分代码的著作权。
