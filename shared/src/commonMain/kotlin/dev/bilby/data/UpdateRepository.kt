...
class UpdateRepository(
    ...
    /** 这个发布附件是不是本机能装的那一个。按平台给,Android 上按 ABI 挑 APK。 */
    private val isInstallableAsset: (assetName: String) -> Boolean,
) {
    ...
    suspend fun check(currentVersion: String): UpdateCheck = runCatching {
        ...
        // 必须挑对架构,规则在平台那边,见 AndroidPlatform.isInstallableUpdateAsset。
        val asset = release.assets.firstOrNull { isInstallableAsset(it.name) }
