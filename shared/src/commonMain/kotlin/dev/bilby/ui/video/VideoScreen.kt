...
import dev.bilby.ui.rememberNotificationPermissionRequest
...
fun VideoScreen(
    ...
    val playerHoldsThisPage = {
        playback.state.value.queue?.frameId == frame.id
    }
    ...
    DisposableEffect(playback) {
        ...
        onDispose {
            controller?.let { connected ->
                ...
                // 发命令,却没问播放器还是不是自己的。
                //
                // **发 [PlaybackCommand.PageLeft] 而不是 pause()。** 后者和用户按下暂停键是同一条路,
                ...
                if (playerHoldsThisPage()) {
                    ...
                    connected.send(PlaybackCommand.FlushProgress)
                    connected.send(PlaybackCommand.PageLeft)
                }
            }
    ...
    val sleepTimerState by playback.sleepTimerState.collectAsStateWithLifecycle()
    ...
    /** 发一条自定义命令的简写。 */
    val send: (PlaybackCommand) -> Unit = { command -> active?.send(command) }
     ...
     */
    val activate: () -> Unit = {
        send(PlaybackCommand.ActivateFrame(frame.id, encodeQueueContext(frame.context), bvid))
    }
    ...
    val jumpToCloudResume: (Long) -> Unit = { positionMillis ->
        ...
        if (cid != 0L && cid != audioState.currentCid) {
            send(PlaybackCommand.PlayPart(cid, positionMillis))
        } else {
    ...
    /** 队列没建全时的重试。 */
    val retryQueue: () -> Unit = { send(PlaybackCommand.RetryQueue) }
    ...
    val queueEdges = audioState.queue?.let { queue ->
        QueueEdges(
            ...
            onLoad = { before -> send(PlaybackCommand.ExtendQueue(before)) },
        )
    }
    ...
    val openDanmakuInput: () -> Unit = {
        ...
        if (playerHoldsThisPage()) send(PlaybackCommand.PageLeft)
        danmakuInputOpen = true
    }

    /**
     * 输入层的唯一出口。各条路径(发送成功、点空白、系统返回、按了发送键、胶囊右端的退出键)都走这里,
    ...
    val onPlayPart: (Long) -> Unit = { cid ->
        send(PlaybackCommand.PlayPart(cid))
    }
    ...
    val setQuality: (Int) -> Unit = { quality ->
        send(PlaybackCommand.SetQuality(quality))
    }

    val setAudioQuality: (Int) -> Unit = { quality ->
        send(PlaybackCommand.SetAudioQuality(quality))
    }

    /** 重试只有一个实现了:流归服务取,它自己就能重来。 */
    val retryPlayback: () -> Unit = { send(PlaybackCommand.Retry) }
    ...
    LaunchedEffect(active, listening) {
        active?.setVideoDisabled(listening)
    }
    ...
    LaunchedEffect(listening) {
        playback.setBackgroundPlaybackAllowed(listening)
    }
    ...
    if (listening && active != null) {
        ...
        ListenScreen(
            ...
            onSleepTimer = { mode -> active.send(PlaybackCommand.SetSleepTimer(mode)) },
            onSleepTimerAdjust = { deltaMinutes -> active.send(PlaybackCommand.AdjustSleepTimer(deltaMinutes)) },
                    ...
                    topBarActions = {
                        ...
                        if (episodeRows.hasSomethingToSwitch()) {
                            PlayerIconButton(
                                ...
                                contentDescription = stringResource(Res.string.video_episodes),
                            )
                        }
                    },
                    embeddedTopActions = {
    ...
    if (expandedLayout) {
        ...
        Column(modifier = Modifier.fillMaxSize().nestedScroll(playerScroll.connection)) {
            ...
            if (!immersive) {
                ...
                val tick by playback.positionTicks.collectAsStateWithLifecycle()
                QuickPlayBar(
                    title = state.detail?.title.orEmpty(),
            ...
            if (parts.isEmpty()) {
                ...
                parts.map { part ->
                    OfflineTarget(
                        item = item,
                        cid = part.cid,
                        partTitle = stringResource(Res.string.video_part_label, part.ordinal, part.title),
                    )
                }
            }
        }
