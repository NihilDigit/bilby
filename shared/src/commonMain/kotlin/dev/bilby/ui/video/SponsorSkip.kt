 ...
 */
val CATEGORY_LABELS: Map<String, StringResource> = mapOf(
    "sponsor" to Res.string.sponsor_sponsor,
    "selfpromo" to Res.string.sponsor_selfpromo,
    "interaction" to Res.string.sponsor_interaction,
    "intro" to Res.string.sponsor_intro,
    "outro" to Res.string.sponsor_outro,
    "preview" to Res.string.sponsor_preview,
    "padding" to Res.string.sponsor_padding,
    "filler" to Res.string.sponsor_filler,
    "music_offtopic" to Res.string.sponsor_music_offtopic,
)
 ...
 * 那九个开关就白给了。
 */
val CATEGORY_DESCRIPTIONS: Map<String, StringResource> = mapOf(
    "sponsor" to Res.string.sponsor_sponsor_desc,
    "selfpromo" to Res.string.sponsor_selfpromo_desc,
    "interaction" to Res.string.sponsor_interaction_desc,
    "intro" to Res.string.sponsor_intro_desc,
    "outro" to Res.string.sponsor_outro_desc,
    "preview" to Res.string.sponsor_preview_desc,
    "padding" to Res.string.sponsor_padding_desc,
    "filler" to Res.string.sponsor_filler_desc,
    "music_offtopic" to Res.string.sponsor_music_offtopic_desc,
)
 ...
 */
val CATEGORY_GROUPS: List<Pair<StringResource, List<String>>> = listOf(
    Res.string.settings_sponsorblock_group_promo to listOf("sponsor", "selfpromo", "interaction"),
    Res.string.settings_sponsorblock_group_structure to listOf("intro", "outro", "preview", "padding"),
    Res.string.settings_sponsorblock_group_content to listOf("filler", "music_offtopic"),
)
...
fun SkipToast(category: String?, modifier: Modifier = Modifier) {
    ...
    val label = stringResource(Res.string.sponsor_skipped, categoryName)
 ...
 * 它不打断任何操作,所以也不必往上取。
 *
 * **画面上那两条一次性提示共用这一个值**([SkipToast] 与 [TripleToast])。它们同形、同位置,
 * 还可能前后脚出现 —— 一条停 4 秒另一条停 2.5 秒,读起来就是第二条"没来得及看清就没了"。
 */
internal const val MediaToastVisibleMillis = 4000L
