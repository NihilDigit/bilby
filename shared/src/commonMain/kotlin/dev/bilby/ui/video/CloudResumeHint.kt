...
/**
 * 这条视频在别处被看到了另一个位置。两种来源:放本地副本时本地那份与云端对不上
 * ([dev.bilby.player.mergeCachedProgress]),以及页面重开时云端被别处写过
 * ([dev.bilby.player.cloudProgressWrittenElsewhere])。
 *
 * **是一条可以无视的提示,不是一次跳转。** 播放已经从自己那份进度起播了,点了才动。自动跳
 * 过去会让播放头在用户没做任何事的情况下自己动,那比停在一个稍旧的位置更难理解 ——
 * 而"稍旧"有下限:它就是本机上次看到的地方。
 *
 * 判据本身在服务那侧,这里只负责摆出来;点下去可能是 seek,也可能是切一 P(别处看的是另一 P),
 * 那个判断在 [VideoScreen] 的 `jumpToCloudResume`。
 *
        ...
        Surface(
            ...
            Row(
                ...
                TextButton(
                    ...
                    Text(stringResource(Res.string.video_cloud_resume_jump))
                }
                TextButton(
                    onClick = {
                        visible = false
                    ...
                    Text(stringResource(Res.string.video_cloud_resume_dismiss))
                }
            }
        }
    }
