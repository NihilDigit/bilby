...
fun FavFolderEditorDialog(state: FavFolderEditorState, actions: FavFolderEditorActions) {
    AlertDialog(
        ...
        title = {
            Text(
                stringResource(
                    if (state.mediaId == null) Res.string.fav_folder_create else Res.string.fav_folder_edit,
                ),
            )
        },
        text = {
            ...
            Column(verticalArrangement = Arrangement.spacedBy(Spacing.Cozy)) {
                OutlinedTextField(
                    ...
                    label = { Text(stringResource(Res.string.fav_folder_title_label)) },
                    singleLine = true,
                ...
                if (!state.isDefaultFolder) {
                    OutlinedTextField(
                        ...
                        label = { Text(stringResource(Res.string.fav_folder_intro_label)) },
                        minLines = 2,
                        maxLines = 4,
                        modifier = Modifier.fillMaxWidth(),
                    )
                ...
                Text(
                    text = stringResource(Res.string.fav_folder_privacy_label),
                    style = MaterialTheme.typography.labelLarge,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
                PrivacyOption(
                    label = stringResource(Res.string.fav_folder_public),
                    selected = state.isPublic == true,
                    onSelect = { actions.onPrivacyChange(true) },
                )
                PrivacyOption(
                    label = stringResource(Res.string.fav_folder_private),
        ...
        dismissButton = {
            TextButton(onClick = actions.onDismiss) { Text(stringResource(Res.string.action_cancel)) }
        },
    )
}
...
fun FavFolderDeleteDialog(state: FavFolderDeletion, actions: FavFolderDeletionActions) {
    AlertDialog(
        ...
        title = { Text(stringResource(Res.string.fav_folder_delete_title)) },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(Spacing.Tight)) {
                Text(stringResource(Res.string.fav_folder_delete_message, state.folder.title, state.folder.count))
                state.error?.let {
                    Text(
                        text = stringResource(it),
                        style = MaterialTheme.typography.bodySmall,
        ...
        confirmButton = {
            TextButton(onClick = actions.onConfirm, enabled = !state.deleting) {
                Text(stringResource(Res.string.fav_folder_delete_confirm))
            }
        },
        dismissButton = {
