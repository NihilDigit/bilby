...
fun StatRow(
    ...
    CompositionLocalProvider(LocalContentColor provides color) {
        Row(
            ...
            playText?.let { Stat(Icons.Outlined.PlayCircle, stringResource(Res.string.stat_play), it) }
            // 弹幕数用自己那个字形,不用字幕框([BilbyIcons])。这一行常常和播放页的控制条同屏,
            // 而那上面的字幕框说的是字幕 —— 同一个符号在一屏里说两件事。
            danmakuText?.let { Stat(BilbyIcons.DanmakuCount, stringResource(Res.string.stat_danmaku), it) }
            dateText?.let {
...
fun formatCount(value: Long): String {
    val large = stringResource(Res.string.count_divisor_large).toLong()
    val small = stringResource(Res.string.count_divisor_small).toLong()
    return when {
        value >= large -> stringResource(Res.string.count_large, value.toDouble() / large)
        value >= small -> stringResource(Res.string.count_small, value.toDouble() / small)
        else -> value.toString()
    }
}
