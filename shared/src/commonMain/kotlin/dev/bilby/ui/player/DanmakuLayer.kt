...
fun PlayerDanmakuLayer(
    ...
    val clock = remember(player, isLive) {
        ...
        if (isLive) LiveDanmakuClock() else SmoothedDanmakuClock(PlayerDanmakuClock(playback))
    }
    ...
    DisposableEffect(player, controller) {
        val listener = object : PlayerListener {
            override fun onIsPlayingChanged(isPlaying: Boolean) = controller.notifyChanged()

            // 覆盖 seek(拖拽松手、双击 ±10 秒、播完重播)。帧循环空闲挂起时只认这一条和兜底
            // 轮询,不接的话 seek 之后弹幕要等最坏 500ms 才跟上。
            override fun onPositionDiscontinuity(oldPositionMillis: Long, newPositionMillis: Long, isSeek: Boolean) =
                controller.notifyChanged()

            override fun onPlaybackSpeedChanged(speed: Float) = controller.notifyChanged()
        }
 ...
 * 状态,和播放页读 [PlaybackHost.state] 是同一个方向。
 */
private class PlayerDanmakuClock(private val playback: PlaybackHost) : DanmakuClock {

    private val tick get() = playback.positionTicks.value
     ...
     *
     * 半秒一条的刻度([PlaybackHost.positionTicks])不满足这一条,消费方必须自己外推,
     ...
     * 不外推**:服务与界面同进程,直接问播放器([PlaybackHost.currentPositionMillis])。
    ...
    override val positionMillis: Long
        get() = playback.currentPositionMillis()
            ?: tick.positionAt(elapsedRealtimeMillis())
