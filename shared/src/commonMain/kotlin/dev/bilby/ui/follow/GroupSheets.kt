    ...
    PaneSheet(onDismissRequest = onDismiss, skipPartiallyExpanded = false) {
        ...
        when {
            state.loading -> InlineProgress(
                stringResource(Res.string.follow_group_loading),
        ...
        SheetActions(
            confirmText = stringResource(Res.string.action_save),
...
fun GroupManagerSheet(
    ...
    PaneSheet(onDismissRequest = onDismiss, skipPartiallyExpanded = false) {
        ...
        LazyColumn(modifier = Modifier.weight(1f, fill = false)) {
                            ...
                            Row {
                                IconButton(onClick = { renaming = group }) {
                                    Icon(
                                        ...
                                        contentDescription = stringResource(
                                            Res.string.follow_group_rename_action,
                                            group.name,
                                        ),
                                    )
                                }
                                IconButton(onClick = { deleting = group }) {
                                    Icon(
                                        ...
                                        contentDescription = stringResource(
                                            Res.string.follow_group_delete_action,
                                            group.name,
                                        ),
                                    )
                                }
            ...
            item(key = "create") {
                ListItem(
                    headlineContent = { Text(stringResource(Res.string.follow_group_create)) },
        ...
        SheetActions(
            confirmText = stringResource(Res.string.action_done),
            confirmEnabled = true,
            onConfirm = onDismiss,
            onDismiss = null,
        )
    ...
    if (creating) {
        GroupNameDialog(
            title = stringResource(Res.string.follow_group_create),
            initial = "",
            onConfirm = {
                creating = false
                onCreate(it)
    ...
    renaming?.let { group ->
        GroupNameDialog(
            title = stringResource(Res.string.follow_group_rename),
            initial = group.name,
            onConfirm = {
                renaming = null
    ...
    deleting?.let { group ->
        AlertDialog(
            ...
            title = { Text(stringResource(Res.string.follow_group_delete_confirm_title, group.name)) },
            confirmButton = {
                TextButton(onClick = {
                    deleting = null
                    onDelete(group.id)
    ...
    AlertDialog(
        ...
        text = {
            OutlinedTextField(
                ...
                label = { Text(stringResource(Res.string.follow_group_name_hint)) },
                singleLine = true,
            )
        },
        confirmButton = {
