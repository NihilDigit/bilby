...
fun BlockConfirmDialog(name: String, onConfirm: () -> Unit, onDismiss: () -> Unit) {
    AlertDialog(
        ...
        title = { Text(stringResource(Res.string.blacklist_block_confirm_title, name)) },
        text = { Text(stringResource(Res.string.blacklist_block_confirm_message)) },
        confirmButton = {
            TextButton(onClick = {
                onDismiss()
                onConfirm()
            }) { Text(stringResource(Res.string.blacklist_block)) }
        },
        dismissButton = {
