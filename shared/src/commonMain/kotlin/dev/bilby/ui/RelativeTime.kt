...
fun formatRelativeTime(epochSeconds: Long, nowEpochSeconds: Long = Instant.now().epochSecond): String {
    ...
    return when {
        diff < 60 -> stringResource(Res.string.time_just_now)
        diff < 3600 -> stringResource(Res.string.time_minutes_ago, diff / 60)
        diff < 24 * 3600 -> stringResource(Res.string.time_hours_ago, diff / 3600)
        diff < 2 * 24 * 3600 -> stringResource(Res.string.time_yesterday)
        diff < 7 * 24 * 3600 -> stringResource(Res.string.time_days_ago, diff / (24 * 3600))
