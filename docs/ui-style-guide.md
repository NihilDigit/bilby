...
### 2.1 三个"选一个"的控件,分工是这样的

这三个长得像,选错了用户学不会规律。判据来自 M3,但落到本项目的具体位置值得记下来:

| 控件 | 什么时候用 | 本项目用在 |
|---|---|---|
| **Segmented button** | 固定的几个选项,切换视图或排序,数量不随内容变 | 搜索的 普通/助理、评论的 最热/最新、空间投稿的 最新/最多播放 |
...
两条 loading 的规矩:**首屏和翻页用不同的粗细**(首屏占整屏,翻页只占一行高度);
**"没有更多了"永远显示**。后者是 DESIGN 1.1 反变比率奖励那一栏的落点 —— 有限集合刷得完,
刷完了就要明确说完了,不能留一个永远转下去的 footer 让人以为还有。
...
- **没有下拉刷新。** 下拉刷新是变比率奖励的标准交互 —— 拉一下,可能有可能没有。
  动态流是时间序的有限集合,滚到底自动翻页,翻完说"没有更多了"。
  `PullToRefreshBox` 在 1.4.0 里是有的,**不要因为"M3 有这个组件"就加上**。
