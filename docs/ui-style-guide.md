...
**楼中楼分两层,照 PiliPlus 的 `replyItemRow` 与 `ReplyItemGrpc(replyLevel: 2)`** ——
那边本来就是两个 widget,不是一个 widget 的两个参数。

- **预览层**(未展开):名字 + `": "` + 正文走同一段文字流,截 2 行,没有头像、时间、属地和
  按钮。整条可点,落点是打开这一组。
- **展开层**(内联展开与面板共用):头像 24dp、名字与标记、时间与属地、正文、配图、
  点赞/回复/删除。缩进两级,组内每条再让出一档。

**这两条推翻了上一版**:楼中楼原先不给头像、正文比主楼小一档。现在正文是 `bodyMedium`
