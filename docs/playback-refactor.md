...
- **D 已落地，真机未验**（commit `0f5f308`、`a65506c`、`5f0094d`，2026-08-13）：
  `player/ProgressSession.kt` 按决定 3 那张表实现全部触发与节流，身份 `(aid, cid)` 创建时冻结，
  `close()` 幂等；aid 由 `BvidCodec.toAid` 从 bvid 换算，不为它取一次详情。上报驱动移进服务，
  `BilbyPlayer` 的 5 秒循环、`VideoScreen` 的 onDispose 补报、`VideoViewModel.reportHeartbeat`
  一并删除，听视频模式由此获得周期心跳。服务发位置刻度（`player/PositionTick.kt`，半秒一条、
  ...
  **顺带修掉一个 B 阶段留下的错误前提**：`LazyMediaSource` 的解析跑在装载周期上，比切条早
  ...
  与设计文档的一处取舍：**离开播放页不再补一条上报**。表里没有这个触发，而周期心跳每 5 秒
  一条，离开时最多丢掉最后几秒；页面那一份反过来只在播放页组合着时才存在，听视频、切后台、
  进程被杀三种它一次都赶不上。
