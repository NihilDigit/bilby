...
没网或从缓存列表进入、而离线副本在盘上则本地源，直播项则直播源，否则在线解析（在线解析后按决定 4 可能换成本地文件）。
...
离开播放页那一条走 `flush()`，**会话不关**——页面走了不等于这次观看结束（听视频、后台连播都
接着走）。它存在的理由是常规心跳有 5 秒节流而暂停本身不发：「暂停 → 退出页面」这条最常见的
路径上，云端会停在最多 5 秒之前，而离开页面正是下一次续播要对准的地方。上报的所有权仍在会话，
页面提供的只是时机，与 seek 落点那一行同构——它给不出位置（播放器归服务），也覆盖不了听视频、
切后台、进程被杀这三种它不在场的情形，那些仍靠周期心跳。命令是 `ACTION_FLUSH_PROGRESS`，
与 `ACTION_PAGE_LEFT` 分开：后者早已不只在离开页面时发，打开发弹幕输入层取的也是它。
...
其位置外推在 `setPlaybackSpeed` 时不刷新锚点，长按倍速瞬间弹幕集体跳动。controller 侧无解，
服务自己发位置刻度是唯一修法，D 阶段已随会话一起落地（`player/PositionTick.kt`）。

### 4. 进度恢复：一个解析器，合并规则保留

恢复只发生在装载时刻，一次解析：用户显式意图 > 云端。离线副本不再是一条独立的装载路径（2026-09-25
改）。有网时一律在线装载，playurl、详情、心跳、弹幕、字幕照常走网络；playurl 回来后，盘上这一 P
（bvid + cid 精确匹配、已下完）的清晰度 id 不低于在线这一次选流会选的那一档，就只把媒体源换成本地
文件，同档时本地优先，编码不比。用户在画质菜单里点过某一档时收紧为正好等于那一档才换。此时菜单列
在线档位，勾的是副本那一档，起播位置按下文的合并规则当场与云端合并。没有网络时不看清晰度，直接放
本地副本、不取 playurl，这一支仍排在任何网络之前判；在线解析或取流失败时退到同一 P 的本地副本。
弱网同样按没网处理：盘上有副本时先问一次 `x/web-interface/nav`，400ms 内没回来就直接放副本，
在线那条不走（弱网下系统照样报有网，只看网络状态挡不住）。
从缓存列表进入是例外：与没网同一支，直接放本地副本，不比清晰度、不取 playurl，起播、切 P、弹幕、
队列都不等网络。弹幕有网先读网络、失败再读盘上那份；没网直接读盘上那份，不等超时。多 P 先问
...
服务端知道的值），`server != base` 取服务端、`== base` 取本地，不取较大值。心跳成功时
本地进度与基线一起推到报上去的值；断网时没发出去的心跳记进待补发表（缓存目录之外），联网后由
WorkManager 补发，补发成功同样推两者。下载时两者以 playurl 带回的服务端记录起头。多 P 边界（在线看过 P2 后离线播 P1，云端参考按 0 计）是服务端单对值
模型的必然结果，不是 bug。

**页面重开（2026-08-13 补）。** 上面那句「恢复只发生在装载时刻」有一个说漏的推论：装载只发生在
冷装载。播放器常驻，同一条视频再打开一次走的是 `openVideo` 的「已经是它了」捷径，一次解析都
不发生，云端一次都没被问——在网页上看到后面某一 P，回到 Bilby 打开还停在旧位置，杀掉 app 反而
正确。根因是那条捷径的幂等是假的：no-op 的判据是「这条已经装着」这个旧状态本身，而不是「现状与
应然一致」这个判断的输出，于是它被自己的脏状态污染。

补的是一次查询加一条提示，不是自动落位：`reconcileCloudProgress` 问一次 `wbi/v2`（单 P 也问，
这条路上比的是位置不只是 P），比对基线是**会话报上去的那一对**（`ProgressSession.reported`），
判据在纯函数 `cloudProgressWrittenElsewhere` 里；不同就摆 `CloudResumeHint`，点了才动，cid 不同
则连 P 一起切过去（同稿件切 P，`ACTION_PLAY_PART` 带上位置）。

**没有选真幂等的那条路**（每次重新解析、结果一致就什么都不做，云端变了自动落位）：它会在用户
毫不知情时把一个暂停中的会话拽到别的位置甚至别的分 P 上，而这一页此刻可能正停在他自己选的地方。
由用户决定，是主人定的。

比对**绝不能拿播放器当前位置当基线**：心跳按位置节流 ≥5 秒，当前位置几乎总比云端新几秒，拿它比
会把自己的节流延迟认成「别处看过」。决定 3 里离开播放页那条 flush 因此是这里的前提——它把云端钉
在一个我们知道的值上。会话把「发出去的」和「确认落地的」分开记，两者任一相等都算自己写的：确认
还没回来的窗口是真实存在的（flush 之后立刻重开页面），而一次失败的上报不能让我们以为服务端变了。
...
- **D 已落地，真机未验**（commit `0f5f308`、`a65506c`、`5f0094d`、`378a2bb`，2026-08-13）：
  `player/ProgressSession.kt` 按决定 3 那张表实现全部触发与节流，身份 `(aid, cid)` 创建时冻结，
  `close()` 幂等；aid 由 `BvidCodec.toAid` 从 bvid 换算，不为它取一次详情。上报驱动移进服务，
  `BilbyPlayer` 的 5 秒循环与 `VideoViewModel.reportHeartbeat` 删除，听视频模式由此获得周期
  心跳。离开播放页那一条留着，但换了归属：页面发 `ACTION_FLUSH_PROGRESS`，位置由会话
  `flush()` 自己取（`378a2bb`）。服务发位置刻度（`player/PositionTick.kt`，半秒一条、
  带锚点），弹幕时钟改读它，倍速回退已修；弹幕分段拉取也改读这条流。
  **顺带修掉一个 B 阶段留下的错误前提**：`LazyMediaSource` 的解析跑在装载周期上，比切条早
  几十秒（`MaskingMediaSource.createPeriod` → `prepareChildSource`，javap 验证），而解析结果
  当时被直接写成「现在放的是什么」——cid、画质清单、页面身份提前跳到下一条，起播 seek 落在正在
  播的那一条上。现在结果先存着，等播放器真的走到那一条再落（`adoptResolved`）。
  **验收留给真机**：连播时上一条要发出 `played_time=-1`，切 P 留下一条定格上报，暂停后退出
  播放页要留下一条停在暂停位置的上报，长按倍速时弹幕不再集体跳动。
...
- **桌面端队列**（2026-09-26）：mpv 一次只装一条，没有 playlist 可借，采用决定 2 的回退形状：
  单条装载，队列语义收进纯状态机 `desktopMain/player/DesktopQueue.kt`。随机顺序移植自 Media3
  `DefaultShuffleOrder`，两端同一套语义。队列栈、补全、续取、分 P 优先的上/下一条、自动连播与
  「播完这条」的规则照搬 Android 服务；轮到哪一条才取哪一条的流，延迟源那一层与 `adoptResolved`
  在桌面没有对应物。放哪一 P 走同一个 `LoadResolver`，本地副本一支暂为空（桌面尚不放本地副本）。
